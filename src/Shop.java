...
public class Shop extends JFrame {
    ...
    public Shop(BoatGame boatGame) {
        ...
        // Ustawienie czcionki na stylizowaną na pixel art
        ...
        // Ustawienia stylu przycisku, aby pasował do pixel art
        sellButton.setBackground(new Color(100, 150, 255)); // Jasny niebieski
        sellButton.setForeground(Color.WHITE);
        sellButton.setFocusPainted(false);
        sellButton.setBorder(BorderFactory.createLineBorder(Color.BLACK, 2));
        add(sellButton, gbc);

        // Panel do rysowania ulepszeń
        ...
        resetUpgradesButton.setBackground(new Color(255, 100, 100)); // Czerwony
    ...
    private void drawHookUpgrade(Graphics g) {
        ...
        for (int i = 0; i < maxHookUpgradeLevel; i++) {
            if (i < hookUpgradeLevel) {
                g.setColor(new Color(0, 200, 0)); // Ciemniejsza zieleń dla pixel art
            } else {
                g.setColor(new Color(150, 150, 150)); // Ciemniejsza szarość
            }
        ...
        int plusY = 30 + 10;
        // Rysowanie pixel-artowego plusa
        g.fillRect(plusX, plusY - 1, 20, 2); // Pozioma linia
        g.fillRect(plusX + 9, plusY - 10, 2, 20); // Pionowa linia
        hookUpgradeButtonRect = new Rectangle(plusX, plusY - 10, 20, 20);

        g.setFont(new Font("Monospaced", Font.PLAIN, 10)); // Mniejsza czcionka dla kosztu
    ...
    private void drawBoatUpgrade(Graphics g) {
        ...
        for (int i = 0; i < maxBoatUpgradeLevel; i++) {
            if (i < boatUpgradeLevel) {
                g.setColor(new Color(0, 0, 200)); // Ciemniejszy niebieski
            } else {
                g.setColor(new Color(150, 150, 150)); // Ciemniejsza szarość
            }
        ...
        g.setColor(Color.BLACK);
        int plusX = 10 + maxBoatUpgradeLevel * 30 + 10;
        int plusY = 90 + 10;
        // Rysowanie pixel-artowego plusa
        g.fillRect(plusX, plusY - 1, 20, 2); // Pozioma linia
        g.fillRect(plusX + 9, plusY - 10, 2, 20); // Pionowa linia
    ...
    private void loadHookUpgradeLevel() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            String line = reader.readLine();
            if (line != null) {
                hookUpgradeLevel = Integer.parseInt(line.trim());
            } else {
                hookUpgradeLevel = 0;
            }
        } catch (IOException | NumberFormatException e) {
    ...
    private void loadBoatUpgradeLevel() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            String line = reader.readLine();
            if (line != null) {
                boatUpgradeLevel = Integer.parseInt(line.trim());
            } else {
                boatUpgradeLevel = 0;
            }
        } catch (IOException | NumberFormatException e) {
    ...
    private void loadHookUpgradeCost() {
        try (BufferedReader reader = new BufferedReader(new FileReader(hookUpgradeCostFile))) {
            String line = reader.readLine();
            if (line != null) {
                hookUpgradeCost = Integer.parseInt(line.trim());
            } else {
                hookUpgradeCost = 200;
            }
        } catch (IOException | NumberFormatException e) {
    ...
    private void loadBoatUpgradeCost() {
        try (BufferedReader reader = new BufferedReader(new FileReader(boatUpgradeCostFile))) {
            String line = reader.readLine();
            if (line != null) {
                boatUpgradeCost = Integer.parseInt(line.trim());
            } else {
                boatUpgradeCost = 200;
            }
        } catch (IOException | NumberFormatException e) {
    ...
    private void displayMessage(String message) {
        ...
        messageFrame.setLocationRelativeTo(this); // Wyśrodkuj względem okna sklepu
